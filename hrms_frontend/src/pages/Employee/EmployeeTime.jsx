...
const EmployeeTime = () => {
  ...
  const fetchLeaveBalances = async () => {
    ...
    try {
      ...
      if (normalized.length > 0) {
        setLeaveBalances(normalized);
      }
    } catch (error) {
      setLeaveBalancesError('Unable to load leave balances right now. Showing default values.');
    } finally {
  ...
  const normalizedGender = (personal?.gender || '').trim().toLowerCase();
  const isFemale = normalizedGender.includes('female');
  const isMale = normalizedGender.includes('male');

  const isLeaveTypeVisible = useCallback((leave) => {
    const type = (leave?.type || '').toLowerCase();
    if (isFemale && type.includes('paternity')) return false;
    if (isMale && type.includes('maternity')) return false;
    return true;
  }, [isFemale, isMale]);
                    ...
                    <div className="row g-3">
                      ...
                      <div className="col-md-6">
                        ...
                        <input
                          ...
                          disabled={!leaveForm.leaveTypeId || isOutOfBalance}
                        />
                        {isLimitedLeave && (
              ...
              <div className="card border-0 shadow-sm h-100 time-card">
                <div className="card-body">
                  <div className="calendar-header">
                    <div>
                      <h5 className="fw-bold mb-1">Holiday Calendar</h5>
                      <div className="text-muted small">{monthLabel}</div>
                    </div>
                    <div className="calendar-actions">
                      <button className="btn btn-outline-secondary btn-sm" type="button" onClick={goPrevMonth}>
                        Prev
                      </button>
                      <button className="btn btn-outline-secondary btn-sm" type="button" onClick={goNextMonth}>
                        Next
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="calendar-grid">
                    {calendarDays.map((date, idx) => {
                      ...
                      return (
                        <div
                          key={key}
                          className={`calendar-cell ${holiday ? 'holiday' : ''} ${holiday?.optional ? 'optional' : ''} ${
                            isToday ? 'today' : ''
                          }`}
                        ...
                        >
                          ...
                          {holiday && (
                            <div className="calendar-holiday">
                              {holiday.name}
                              <span className="calendar-tag">{holiday.optional ? 'Optional' : 'Holiday'}</span>
                            </div>
                          )}
                        </div>
                      );
              ...
              <div className="card border-0 shadow-sm h-100 time-card">
                <div className="card-body">
                  <div className="holiday-summary-header">
                    <div>
                      <h5 className="fw-bold mb-1">Yearly Holiday Chart</h5>
                      <div className="text-muted small">
                        {currentYear} overview | {yearlyHolidays.length} holidays
                      </div>
                    </div>
                  </div>
                  <div className="holiday-list">
                    {sortedHolidays.map((holiday) => (
                      ...
                      >
                        ...
                        <div className="holiday-tag">{holiday.optional ? 'Optional' : 'Holiday'}</div>
                      </div>
                    ))}
                  </div>
                </div>
